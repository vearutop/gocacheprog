...
//
// Source: https://tip.golang.org/src/cmd/go/internal/cacheprog/cacheprog.go?m=text.
package cacheprog
...
type Request struct {
	...
	ActionID string `json:",omitempty"` // or nil if not used

	// OutputID is stored with the body for "put" requests.
	OutputID string `json:",omitempty"` // or nil if not used
