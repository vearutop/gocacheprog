...
func TestStatus(t *testing.T) {
	...
	require.True(t, ok)
	require.NotContains(t, storeStats, "diskBytes", "raw diskBytes is replaced by the human-readable storage field")
	require.NotContains(t, storeStats, "maxDiskBytes", "per-store budgets are rarely set in server mode; see the combined-budget line instead")
	require.NotContains(t, storeStats, "maxDiskBytesHuman")
	require.NotContains(t, storeStats, "lastEviction")
	require.Contains(t, storeStats, "storage")

	gocacheStats, ok := body["gocache"].(map[string]any)
	require.True(t, ok)
	require.NotContains(t, gocacheStats, "diskBytes")
	require.NotContains(t, gocacheStats, "maxDiskBytes")
	require.NotContains(t, gocacheStats, "maxDiskBytesHuman")
	require.NotContains(t, gocacheStats, "lastEviction")
	require.Contains(t, gocacheStats, "storage")
	require.NotContains(t, gocacheStats, "misses", "the native GOCACHE store has no get/miss concept")
